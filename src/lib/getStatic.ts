...
export async function getI18nProps(
  ...
  const siteName =
    locale === 'ja' ? process.env.sitenameJA : process.env.sitenameEN

  const ogImage = img
    ? `https://${process.env.domain}${img}`
    : `https://${process.env.domain}/ogp.jpg`
  ...
  const seoData: SeoData[] = [
    ...
    {
      property: 'og:url',
      content: `https://${process.env.domain}/${locale}${pathname}`,
    },
